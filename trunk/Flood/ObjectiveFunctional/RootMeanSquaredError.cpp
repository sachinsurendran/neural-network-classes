...
RootMeanSquaredError::RootMeanSquaredError(MultilayerPerceptron* newMultilayerPerceptron, 
...
/// This method returns a pointer to the input-target object on which the objective functional is measured.

InputTargetDataSet* RootMeanSquaredError::getInputTargetDataSet(void)
{
...
double RootMeanSquaredError::calculateEvaluation(void)
{
   ...
   {
      std::cerr << std::endl
                << "Flood Error: RootMeanSquaredError class." << std::endl
...
/// This method returns the the root mean squared error function gradient of a multilayer perceptron on an 
/// input-target data set. It uses the error back-propagation method.
///
/// @see calculateEvaluation(void).

Vector<double> RootMeanSquaredError::calculateGradient(void)
{
   // Multilayer perceptron 
   ...
   for(int sample = 0; sample < numberOfSamples; sample++)
   {
      ...
      switch(preAndPostProcessingMethod)
      {
         ...
         {
            outputError = outputSignalDerivative*(output-target)/(evaluation*(double)numberOfSamples);
         }// end none
         ...
         {
            outputError = outputSignalDerivative*((output-target)*standardDeviationOfOutputVariables)
            /(evaluation*numberOfSamples);
         ...
         case MultilayerPerceptron::MinimumAndMaximum:
         {
            ...
            /(2.0*evaluation*numberOfSamples);
...
void RootMeanSquaredError::saveInputTargetAndOutput(char* filename)
{
   int numberOfOutputs = multilayerPerceptron->getNumberOfOutputs();
   ...
   file << "% Flood Neural Network. Input-target-output data file."
        << std::endl
