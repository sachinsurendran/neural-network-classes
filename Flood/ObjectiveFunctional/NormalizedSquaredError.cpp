...
NormalizedSquaredError::NormalizedSquaredError(MultilayerPerceptron* newMultilayerPerceptron, 
...
: ObjectiveFunctional(newMultilayerPerceptron)
{
   ...
   meanOfTargetData = meanAndStandardDeviationOfTargetData.getRow(0);
}
...
InputTargetDataSet* NormalizedSquaredError::getInputTargetDataSet(void)
{
...
void NormalizedSquaredError::setInputTargetDataSet(InputTargetDataSet* newInputTargetDataSet)
{
   inputTargetDataSet = newInputTargetDataSet;
...
double NormalizedSquaredError::calculateEvaluation(void)
{
    // Control sentence 
   ...
   {
      std::cerr << std::endl
                << "Flood Error: NormalizedSquaredError class." << std::endl
   ...
   for(int sample = 0; sample < numberOfSamples; sample++)
   {
      ...
      sumSquaredNorm += (target-meanOfTargetData).dot(target-meanOfTargetData);
   }
   normalizedSquaredError = sumSquaredError/sumSquaredNorm;
...
Vector<double> NormalizedSquaredError::calculateGradient(void)
{
   // Multilayer perceptron 
   ...
   gradient = gradient/sumSquaredNorm;
...
void NormalizedSquaredError::saveInputTargetAndOutput(char* filename)
{
   int numberOfOutputs = multilayerPerceptron->getNumberOfOutputs();
