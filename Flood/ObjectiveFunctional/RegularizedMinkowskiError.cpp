...
/*   www.cimne.com/flood                                                                                        */
/*                                                                                                              */
/*   R E G U L A R I Z E D   M E A N   S Q U A R E D   E R R O R   C L A S S                                    */
/*                                                                                                              */
/*   Roberto Lopez                                                                                              */
...
#include "RegularizedMinkowskiError.h"
...
/// General constructor. It creates a regularized mean squared error objective functional object associated to a 
...
/// <li> Epsilon: 1.0e-6.
/// <li> Display = true. 
/// <li> Mean squared error weight = 1.0.
/// <li> Regularization weight = 0.1.
/// </ul> 
///
...
RegularizedMinkowskiError::RegularizedMinkowskiError(MultilayerPerceptron* newMultilayerPerceptron, 
InputTargetDataSet* newInputTargetDataSet)
: ObjectiveFunctional(newMultilayerPerceptron)
{
   inputTargetDataSet = newInputTargetDataSet;

   meanSquaredErrorWeight = 1.0;
...
/// Default constructor. It creates a regularized mean squared error objective functional object not associated to
/// any multilayer perceptron and not measured on any input-target data set.
/// It also initializes all the rest of class members to their default values:
///
/// <ul>
/// <li> Number of evaluations = 0. 
...
RegularizedMinkowskiError::RegularizedMinkowskiError(void) : ObjectiveFunctional()
{
...
RegularizedMinkowskiError::~RegularizedMinkowskiError(void)
{
...
InputTargetDataSet* RegularizedMinkowskiError::getInputTargetDataSet(void)
{
   return(inputTargetDataSet);
}


// double getMeanSquaredErrorWeight(void) method

/// This method returns the weight of the error term in the objective functional expression.

double RegularizedMinkowskiError::getMeanSquaredErrorWeight(void)
{
   return(meanSquaredErrorWeight);
}


// double getRegularizationWeight(void) method

/// This method returns the weight of the regularization term in the objective functional expression.

double RegularizedMinkowskiError::getRegularizationWeight(void)
{
...
void RegularizedMinkowskiError::setInputTargetDataSet(InputTargetDataSet* newInputTargetDataSet)
{
   inputTargetDataSet = newInputTargetDataSet;
}


// void setMeanSquaredErrorWeight(double) method

/// This method sets a new weight for the error term in the objective functional expression.
/// This value must be greater than zero. 
///
/// @param newMeanSquaredErrorWeight Weight value for the error term. 

void RegularizedMinkowskiError::setMeanSquaredErrorWeight(double newMeanSquaredErrorWeight)
{
   // Control sentence 

   if(newMeanSquaredErrorWeight <= 0.0)
   {
      std::cout << std::endl
                << "Flood Error RegularizedMinkowskiError class." << std::endl
                << "void setMeanSquaredErrorWeight(double) method." << std::endl
                << "The mean squared error term weight must be greater than zero." 
                << std::endl << std::endl;

      exit(1);
   }

   // Set mean squared error term weight

   meanSquaredErrorWeight = newMeanSquaredErrorWeight;
}


// void setRegularizationWeight(double) method

/// This method sets a new weight for the regularization term in the objective functional expression.
/// This value must be equal or greater than zero.
///
/// @param newRegularizationWeight Weight value for the regularization term. 

void RegularizedMinkowskiError::setRegularizationWeight(double newRegularizationWeight)
{
   // Control sentence 
   ...
   // Set regularization term weight
...
/// This method returns the evaluation value of a multilayer perceptron according to the mean squared error on an 
/// input-target data set.
///
/// @see calculateGradient(void).

double RegularizedMinkowskiError::calculateEvaluation(void)
{
   ...
   double RegularizedMinkowskiError = 0.0;       
   ...
   double meanSquaredError = sumSquaredError/(double)numberOfSamples;

   double sumSquaredNeuralParameters = neuralParameters.dot(neuralParameters);

   double meanSquaredNeuralParameters = sumSquaredNeuralParameters/(double)numberOfNeuralParameters;

   RegularizedMinkowskiError = meanSquaredErrorWeight*meanSquaredError 
   + regularizationWeight*meanSquaredNeuralParameters;

   return(RegularizedMinkowskiError);
}
...
Vector<double> RegularizedMinkowskiError::calculateGradient(void)
{
   // Multilayer perceptron 
   ...
   for(int sample = 0; sample < numberOfSamples; sample++)
   {
      ...
      switch(preAndPostProcessingMethod)
      {
         ...
         {
            outputError = outputSignalDerivative*(output-target)*2.0/(double)numberOfSamples;
         }
   ...
   gradient = gradient*meanSquaredErrorWeight + neuralParameters*regularizationWeight*2.0/(double)numberOfNeuralParameters;
...
void RegularizedMinkowskiError::saveInputTargetAndOutput(char* filename)
{
   int numberOfOutputs = multilayerPerceptron->getNumberOfOutputs();
   ...
   Vector<double> input(numberOfInputVariables);
   Vector<double> output(numberOfTargetVariables);
