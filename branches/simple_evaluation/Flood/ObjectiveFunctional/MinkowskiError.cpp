...
MinkowskiError::MinkowskiError(MultilayerPerceptron* newMultilayerPerceptron, 
...
InputTargetDataSet* MinkowskiError::getInputTargetDataSet(void)
{
...
double MinkowskiError::getMinkowskiParameter(void)
{
...
void MinkowskiError::setInputTargetDataSet(InputTargetDataSet* newInputTargetDataSet)
{
   inputTargetDataSet = newInputTargetDataSet;
}
...
///
/// @param newMinkowskiParameter Minkowski exponent value. 

void MinkowskiError::setMinkowskiParameter(double newMinkowskiParameter)
{
   // Control sentence
   ...
   {
      std::cerr << std::endl
                << "Flood Error. MinkowskiError class." << std::endl
...
double MinkowskiError::calculateEvaluation(void)
{
    // Control sentence 
   ...
   for(int sample = 0; sample < numberOfSamples; sample++)
   {
     ...
     target = targetData.getRow(sample);
      ...
      {
         error = fabs(output[j] - target[j]);
           
         minkowskiError += pow(error, minkowskiParameter);
      }
   }
...
Vector<double> MinkowskiError::calculateGradient(void)
{
   ...
   for(int sample = 0; sample < numberOfSamples; sample++)
   {
      ...
      switch(preAndPostProcessingMethod)
      {
         case MultilayerPerceptron::None:
         {
            for(int j = 0; j < numberOfOutputs; j++)
            {
               if(output[j] - target[j] >= 0.0)
                {
                   outputError[j] = outputSignalDerivative[j]
                   *minkowskiParameter*pow(fabs(output[j] - target[j]), minkowskiParameter-1.0);
                }
                else
                {
                   outputError[j] = outputSignalDerivative[j]
                   *(-1.0)*minkowskiParameter*pow(fabs(output[j] - target[j]), minkowskiParameter-1.0);                 
                }
            }	 }//end none
         break;
         ...
         case MultilayerPerceptron::MeanAndStandardDeviation:
         {
            for(int j = 0; j < numberOfOutputs; j++)
            {
               if(output[j] - target[j] >= 0.0)
               {
                  outputError[j] = outputSignalDerivative[j]
                  *minkowskiParameter*pow(fabs(output[j] - target[j]), minkowskiParameter-1.0)
                  *standardDeviationOfOutputVariables[j];
               }
               else
               {
                  outputError[j] = outputSignalDerivative[j]
         ...
         }// end meand and standard deviation         
         break;
         ...
         case MultilayerPerceptron::MinimumAndMaximum:
         {
            for(int j = 0; j < numberOfOutputs; j++)
            {
               if(output[j] - target[j] >= 0.0)
               {
                  outputError[j] = outputSignalDerivative[j]
                  *minkowskiParameter*pow(fabs(output[j] - target[j]), minkowskiParameter-1.0)
                  *0.5*(maximumOfOutputVariables[j]-minimumOfOutputVariables[j]);
               }
               else
               {
                  outputError[j] = outputSignalDerivative[j]
...
/// This method saves to a data file the inputs and the targets in the input-target data set,
/// together with the outputs from the multilayer perceptron for that inputs. 
///
/// @param filename Filename.

void MinkowskiError::saveInputTargetAndOutput(char* filename)
{
   int numberOfInputs = multilayerPerceptron->getNumberOfInputs();
   int numberOfOutputs = multilayerPerceptron->getNumberOfOutputs();
